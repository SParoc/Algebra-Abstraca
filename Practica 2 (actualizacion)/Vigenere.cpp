...
string Vigenere::cipherText(string plainText, string cipherText, string key, bool mod, int op)
{
	if (op == 1)
	{
		abc = num;
...
string Vigenere::decipherText(string cipherText, string plainText, string key, bool mod, int op)
{
	if (op == 1)
	{
		abc = num;
	...
	for (int i = 0; i < cipherText.length(); i++)
	{
		...
		if (mod && k2)
		{
			posKey = abc.find(decipherText(cipherText, plainText, key, false,op)[k]);
			k += 1;
			if (k >= key.length())
			{
				k = 0;
